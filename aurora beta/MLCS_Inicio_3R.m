...
while toc(t.atual) < t.tmax
    ...
    % Alterando a formação desejada dinamicamente
    r = 1;
    FT.pPos.qd(1) = r*sin(2*pi*toc(t.atual)/t.tmax);
    FT.pPos.qd(2) = r*sin(4*pi*toc(t.atual)/t.tmax);
    FT.pPos.qd(4) = pi*sin(2*pi*toc(t.atual)/t.tmax);
    
    FT.pPos.dqd(1) = r*cos(2*pi*toc(t.atual)/t.tmax)*2*pi/t.tmax;
    FT.pPos.dqd(2) = r*cos(4*pi*toc(t.atual)/t.tmax)*4*pi/t.tmax;
    FT.pPos.dqd(4) = pi*cos(2*pi*toc(t.atual)/t.tmax)*2*pi/t.tmax;
