...
%          for ii = 1:length(Madj)
%             if ii == 1 && toc(t) 
%                 P{ii}.pPos.X(1) = cos(2*pi*0.025*toc(t));
%                 P{ii}.pPos.X(2) = sin(2*pi*0.025*toc(t));
%                 P{ii}.pPos.X(7) =-2*pi*0.025*sin(2*pi*0.025*toc(t));
%                 P{ii}.pPos.X(8) = 2*pi*0.025*cos(2*pi*0.025*toc(t));
%             end
%             
...
%             C.mEstadoInformacaoPopular(P{ii});
    ...
    if toc(tp) > 0.2
        ...
        disp(['area: ' num2str(area(alphaShape(x1(1:4),y1(1:4),2.5)))]);
        
        x = P{1}.pPos.X(1)+[1; 0; -1; 0; 1];
        y = P{1}.pPos.X(2)+[0; 1; 0; -1; 0];     
        h3 = plot(x, y, 'b--'); 
        drawnow
    end
